...
parser.add_argument("--threshold", type=float, default=0.35, help="minimum detection threshold to use")
...
while True:
    ...
    if len(poses)<1:
        stop()
    print("these are poses",poses)
    tracked_pose = None
    min_distance = float('inf')
    #print("detected {:d} objects in image".format(len(poses)))
    if weapon_detected and bodycount==0:
