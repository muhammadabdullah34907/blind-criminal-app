...
while True:
    ...
    # Get bounding boxes from poses
    bboxes = np.array([[int(p.Left), int(p.Top), int(p.Right), int(p.Bottom), 1.0] for p in poses])
    ...
    for track in track_bbs_ids:
        ...
        cudaDrawRect(img, (int(x1), int(y1), int(x2), int(y2)), (255, 0, 0, 200))
        #font.OverlayText(img, f'ID: {int(track_id)}', int(x1), int(y1), fontSize=10, color=(255, 0, 0, 255))

    # Control PTZ based on the tracked ID
    if len(track_bbs_ids) > 0:
        # Assuming tracking the first detected ID
        tracked_bbox = track_bbs_ids[0]
        center_x = int((tracked_bbox[0] + tracked_bbox[2]) / 2)
        center_y = int((tracked_bbox[1] + tracked_bbox[3]) / 2)
        frame_center = (img.width // 2, img.height // 2)
        track_keypoint(frame_center, (center_x, center_y))
